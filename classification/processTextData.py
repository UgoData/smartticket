...
class ProcessText:
    ...
    def clean_text_data(df):
        ...
        df['merge_final'] = df['merge_cols_simpl'] + ' ' + df['merge_col_3'] + ' ' + df['merge_col_4'] + ' ' + df[
            'merge_col_5'] + ' ' + df['brands_tags']
        # Suppress double words and keep order
        df['merge_final'].fillna('', inplace=True)
        df['merge_final'] = df['merge_final'].map(lambda x: n.clean_duplicate_string(x))
        return df
